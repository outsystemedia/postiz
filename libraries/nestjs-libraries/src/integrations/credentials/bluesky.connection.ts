...
async function agentFor(fields: Record<string, string>) {
  ...
  try {
    ...
    throw new CredentialError(
      'App password inválida ou identificador incorreto. Não use a senha principal.'
    );
  }
  return agent;
}
export const blueskyConnection: CredentialConnection = {
  ...
  async authenticate(fields) {
    const credentials = {
      service: await serviceUrl(fields.service || 'https://bsky.social'),
      identifier: required(fields, 'identifier', 253).trim(),
      password: required(fields, 'password', 19),
    };
    if (!/^[a-zA-Z0-9]{4}(?:-[a-zA-Z0-9]{4}){3}$/.test(credentials.password))
      throw new CredentialError(
        'App password inválida. Use o formato xxxx-xxxx-xxxx-xxxx; não use a senha principal.'
      );
  ...
  async post(provider, auth, posts, integration, reply) {
    ...
    return reply
      ? BlueskyProvider.prototype.comment.call(
          context,
          auth.id,
          reply.root,
          reply.last,
      ...
      : BlueskyProvider.prototype.post.call(
          context,
          auth.id,
          '',
          posts,
