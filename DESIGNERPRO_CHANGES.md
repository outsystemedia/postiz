...
| `libraries/nestjs-libraries/src/temporal/temporal.module.ts` | **Modified.** Makes the orchestrator fail closed when its native Temporal worker connection cannot be created, allowing PM2 to retry instead of leaving a superficially online process with no queue pollers. | 2026-09-23 |
| `apps/orchestrator/src/health.controller.ts` | **Modified.** Requires all configured workers, including WordPress, to be initialized, running and healthy before returning HTTP 200. | 2026-09-23 |
| `apps/orchestrator/src/health.controller.spec.ts` | **New file.** Regression coverage for healthy and workerless-degraded orchestrator states. | 2026-09-23 |
...
| `docker-compose.designerpro.yaml` | **Modified.** Adds a container healthcheck against the orchestrator's worker-aware endpoint, so a public HTTP process with absent publication workers is exposed as degraded. | 2026-09-23 |
...
| `jest.config.designerpro.ts` | **Modified.** Includes the orchestrator health regression test and source alias. | 2026-09-23 |
