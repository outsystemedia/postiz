...
export const devtoConnection: CredentialConnection = {
  ...
  async authenticate(fields) {
    ...
    const me = await credentialJson(
      ...
      'API Key Dev.to inválida. Gere uma chave em Settings → Extensions.'
    );
    if (!me.id || !me.username)
      throw new CredentialError(
        'A API Key não retornou uma conta Dev.to válida.'
      );
    return {
      id: String(me.id),
  ...
  async post(provider, auth, posts, integration, reply) {
    if (reply)
      throw new CredentialError(
        'Dev.to não suporta respostas automáticas nesta conexão.'
      );
    return DevToProvider.prototype.post.call(
