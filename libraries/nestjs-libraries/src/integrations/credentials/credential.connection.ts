...
import {
  ...
  PostResponse,
  SocialProvider,
} from '../social/social.integrations.interface';
...
export type CredentialEnvelope = CredentialIdentity & {
  version: 1;
  provider: string;
};
...
export type CredentialConnection = {
  ...
  post(
    ...
    auth: CredentialEnvelope,
    posts: PostDetails[],
    integration: Integration,
    reply?: { root: string; last?: string }
...
export function required(
  ...
  if (typeof value !== 'string' || !value.trim() || value.length > max)
    throw new CredentialError('Preencha os campos de conexão corretamente.');
  return value;
}
export async function serviceUrl(value: string): Promise<string> {
  ...
  try {
    ...
    throw new CredentialError('Informe uma URL HTTPS válida para a instância.');
  }
  if (
    url.username ||
    url.password ||
    ...
    url.hash ||
    url.pathname !== '/' ||
    !(await isSafePublicHttpsUrl(url.href))
  ) {
    throw new CredentialError(
      'Informe uma instância HTTPS pública, sem caminho, usuário ou senha na URL.'
    );
  }
  return url.origin;
}
...
export async function credentialFetch(
  ...
  if (url.username || url.password || !(await isSafePublicHttpsUrl(url.href)))
    throw new CredentialError('O endereço do serviço não é permitido.');
  try {
    ...
    throw new CredentialError(
      'Não foi possível acessar o serviço. Verifique o endereço e tente novamente.',
      502
    );
  }
}
export async function credentialJson(
  url: string,
  init: RequestInit = {},
  invalid = 'Token inválido ou sem permissão.'
): Promise<any> {
  ...
  try {
    ...
    throw new CredentialError('O serviço retornou uma resposta inválida.', 502);
  }
}
export function readCredentialEnvelope(
  ...
  if (
    ...
    throw new CredentialError(
      'Reconecte este canal para atualizar suas credenciais.'
    );
  return value;
}
export function withCredentialConnection<T extends ExistingProvider>(
  provider: T,
  connection: CredentialConnection
): T & { credentialConnection: CredentialConnection } {
  ...
  wrapped.post = async (id, token, posts, integration) => {
    if (!token.startsWith('secret:v1:'))
      return legacyPost(id, token, posts, integration);
    try {
      const result = await connection.post(
        provider,
        readCredentialEnvelope(token, provider.identifier),
        posts,
        integration
      );
      if (
        !result.length ||
        result.some(
          (post) => !post.postId || String(post.postId) === 'undefined'
        )
      )
        throw new CredentialError('O serviço não confirmou a publicação.');
      return result;
    } catch (err) {
      throw new BadBody(
        provider.identifier,
        ...
        err instanceof CredentialError
          ? err.message
          : 'Não foi possível publicar. Verifique as credenciais e permissões do canal.'
      );
    }
  };
  wrapped.comment = async (id, root, last, token, posts, integration) => {
    if (!token.startsWith('secret:v1:'))
      return legacyComment
        ? legacyComment(id, root, last, token, posts, integration)
        : [];
    try {
      ...
      throw new BadBody(
        ...
        err instanceof CredentialError
          ? err.message
          : 'Não foi possível publicar a resposta. Verifique o canal.'
      );
    }
  };
  return wrapped;
...
export function legacyContext(provider: ExistingProvider) {
  ...
  context.fetch = async (url: string, init?: RequestInit) => {
    ...
    if (!response.ok)
      throw new CredentialError(
        'Não foi possível publicar. Verifique as permissões e o conteúdo.'
      );
    return response;
  };
  return context;
